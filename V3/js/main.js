...
// ============================================================
// 🏆 Affichage du classement des amis
// ============================================================
const rankingBtn = document.getElementById("rankingBtn");
const rankingSection = document.getElementById("rankingSection");
...
if (rankingBtn && rankingSection) {
  rankingBtn.addEventListener("click", async () => {
    // Toggle d’affichage
    const isVisible = rankingSection.style.display === "block";
    rankingSection.style.display = isVisible ? "none" : "block";
    if (isVisible) {
      rankingSection.innerHTML = "";
      return;
    }
    ...
    rankingSection.innerHTML = `
      <div class="ranking-card">
        <h3>🏆 <strong>Classement des amis</strong></h3>
        <p>Chargement du classement...</p>
      </div>
    `;

    try {
      const response = await fetch(`${CONFIG.API_URL}?action=getRanking`);
        ...
        .map(
          (p, i) =>
            `<div class="ranking-row">
               ...
               <span class="points">${p.totalPoints} pts</span>
      ...
      rankingSection.innerHTML = `
        <div class="ranking-card">
          <h3>🏆 <strong>Classement des amis</strong></h3>
          <div class="ranking-list">${topPlayers}</div>
        </div>
      `;
    } catch (err) {
      console.error("❌ Erreur de chargement du classement :", err);
      rankingSection.innerHTML = `
        <div class="ranking-card error">
          <h3>❌ Erreur</h3>
